...
% 日本語情報（必要なら）
\jclass	{卒業論文}							% 論文種別
\jtitle		{DreamDate：レム睡眠時における音刺激の提示を用いた明晰夢制御システム}			% タイトル。改行する場合は\\を入れる
...
\jhyeared {27}								% 平成○年「度」
\jsyear	{2015}						% 西暦○年
\jsyeared	{2015}							% 西暦○年「度」
\jkeyword	{Lucid Dreaming, iOS, Sleep Monitoring}			% 論文のキーワード

% 英語情報（必要なら）
\eclass	{Graduation Thesis}					% 論文種別
\etitle		{DreamDate: lucid dream control system\\ using sound stimuli during REM sleep}	% タイトル。改行する場合は\\を入れる
